"""Run the bipartite GapPTR simulation while varying the number of right nodes.
...
try:
    ...
    from bipartite_gaptr_utils import (  
        add_common_args,
        default_m_values,
        plot_summary,
        run_one_replication,
...
def build_argparser() -> argparse.ArgumentParser:
    """Build command-line parser for the varying-m experiment."""

    parser = argparse.ArgumentParser(
        description=(
            "Bi-DCSBM simulation for bipartite GapPTR: vary m and compare "
            "NonPrivate against GapPTR under several eps1 splits."
        )
    )
    ...
    parser.add_argument(
        ...
        help="Grid of right-side node counts m.",
    )
    parser.add_argument(
        "--eps_total",
        type=float,
        default=0.8,
        help="Total privacy budget eps_total = eps_main + eps1.",
    )
    parser.add_argument(
        "--no_ci",
        action="store_true",
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if any(m <= 0 for m in args.m_values):
        raise ValueError("All m_values must be positive.")
    if args.eps_total <= 0:
    ...
    if max_eps1 >= args.eps_total:
        raise ValueError(
            "The largest eps1 value must be smaller than eps_total. "
            f"Got max eps1={max_eps1} and eps_total={args.eps_total}."
        )
...
def main() -> None:
    ...
    total_jobs = len(args.m_values) * args.reps
    job_id = 0
    ...
    summary_df = summarize_records(raw_df, x_col="m")

    raw_path = outdir / "vary_m_raw.csv"
    summary_path = outdir / "vary_m_summary.csv"
    figure_path = outdir / "vary_m_clustering_error.png"
    ...
    plot_summary(
        ...
        x_col="m",
        title=(
            f"Bi-DCSBM GapPTR: varying m "
            f"(scenario={args.scenario}, n={args.n}, eps_total={args.eps_total:g})"
        ),
        xlabel="m (number of right-side nodes)",
        outpath=figure_path,
        log_x=False,
